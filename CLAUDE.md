...
# (the spoken translation, then the marked review); the reverse challenge
# narrates "translation_into_target.prompt" and plays that block's attempt in
# the language being learned. The manuscript itself is
...
2. one side of that lesson is narrated once, as in Narrate;
3. the student presses the 🎙 button for the *other* language and **translates the story aloud**; that turn is transcribed and marked against the answer key (`translation_challenge.review_translation()`, saved as `translation_review.md`/`.json`, spoken back as a short summary and shown in full in the chat bubble);
4. push-to-talk then continues as usual, with the story and the review in the tutor's system prompt (`session_core.build_challenge_system_prompt()`) so "why is it *le* in sentence four" is answered from the text the student just worked through. Speaking the *other* language during step 3 is read as "I'd rather just talk" and skips straight here;
5. Exit runs Analyze + Homework + persist, with the review's findings included.

The challenge runs in either **direction**, chosen by which landing button was pressed and carried as `direction` on the `start_challenge` message through the setup graph into the lesson:

- `into_native` (📝 Translation challenge) — the target-language story is narrated and the student says what it means in their own language. This tests **comprehension**: their native language is only the readout, so every deviation is read as a question about the target language.
- `into_target` (🔁 Reverse challenge) — the same story's natural translation is narrated in the student's own language and they say it back in the one they are learning. This tests **production**: everything they utter is under test — gender and agreement, tense and mood, prepositions, word order, word choice.

One build serves both. The story is always written in the target language (level and grammar focus only mean something applied to the language being learned) and glossed sentence by sentence into the native one; the reverse direction hands the student the `Nat.` lines joined into prose — plus a translated title — and marks their speech against the target sentences those lines came from, so the answer key is aligned by construction. `lesson["prompt"]` is the side the student is given (`translation_challenge.prompt_side()`) and `lesson["speak_lang"]` the language they answer in; the `stage` WebSocket message carries that language so the frontend greys out the button that would skip the exercise, and narration is saved as `story_<narrated language>.wav`. `review_translation()` dispatches on the direction: the comprehension prompt reads every deviation as a misreading, the production prompt marks the target language itself and additionally gets the focus pattern, which names the mistake a learner reliably makes with the drilled structure. Both share the judging filters, the pair's trap catalogue (read from the other side — a speaker producing the target language flattens the same distinctions they misread), the JSON schema, the three verdicts and everything downstream. Which way a session went is recorded in the profile's `practice_log` as `direction`, since two sessions on one focus are not the same practice if only one asked the student to produce it.

`lesson.md` is deliberately not linked until the translation has been marked — it contains the answer key. In a reverse challenge it opens with the native-language prose the student heard, and everything below it is that text's answer key. The two artifacts are then offered in the chat (an `artifact` WebSocket message) and again on the completion screen.

`translation_challenge.py` owns the prompts, JSON schemas and markdown rendering for both halves; `skill_refs.py` feeds them by pulling the *exact* sections the skills tell a reader to consult out of `.claude/skills/**/references/` — the CEFR band for the level, the forms the target language admits at that level, the focus pattern being drilled, the glossing notation and its worked examples, the judging filters, the pair's trap catalogue. Two axes select the files: the **target** language alone gives forms, focus patterns and glossing notes (`language-lesson/references/languages/<target>.md`), while the **pair** gives the comprehension traps (`translation-review/references/languages/<target>-<native>.md`), because what a Spanish speaker misreads in French is not what an English speaker misreads in Spanish. The skill files stay the single source of truth: editing `references/languages/fr.md` changes what the app generates with no code change, and a lesson written by Claude and one generated by the app are calibrated to the same scale. The work is split into small separately-validated Ollama calls (story, then one gloss per sentence, then — in the reverse direction — the title, then notes) because llama3.1:8b reliably drops half the output when asked for all of it at once; the glossing pass is why building a challenge takes about a minute, and the progress callback threaded through the graph's run config exists so that minute doesn't look like a hang.
...
the prompts out of these skills' own reference files. Its reverse direction (the student
producing the target language against the same answer key) has no skill counterpart; it
reuses `translation-review`'s judging filters and pair traps, read from the other side.
Use the skills when you want Claude's
